...
void ggint_mod_gint2( gint a,  gint b, gint r)
{
    ...
    if (b[GInt_Size-1] & 0x80)         // если отрицательное 
    {
        ...
        ggint_positive_to_negative(r); // а потом r = -r (чтобы было как в C++)
        ggint_positive_to_negative(b); // вернуть как было
    } else
...
void ggint_pow_mod(gint a, gint x,  gint n, gint r)
{
    ...
    {
        if (ggint_is_odd(x)) // если нечетно
        {
            ggint_mul_gint(a, r, t); // t = a * r
            ggint_mod_gint(n, t, r); // r = t % n
        }
        ggint_shbr(x, 1);        // x /= 2
        ggint_mul_gint(a, a, t); // t = a^2
        ggint_mod_gint(n, t, a); // a = t % n // его
    }
}
